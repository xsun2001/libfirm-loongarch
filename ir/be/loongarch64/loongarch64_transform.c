...
static ir_node *transform_const(ir_node *const node, ir_entity *const entity, ir_tarval *const value)
{
	...
	return new_bd_loongarch64_Const(dbgi, block, entity, value);
}
...
{
	return transform_binop(node, new_bd_loongarch64_And);
}

static ir_node *gen_Or(ir_node *node)
{
	return transform_binop(node, new_bd_loongarch64_Or);
}

static ir_node *gen_Eor(ir_node *node)
{
	return transform_binop(node, new_bd_loongarch64_Xor);
}
...
#endif
	return transform_binop(node, new_bd_loongarch64_fDiv);
}
...
static ir_node *gen_Shl(ir_node *node)
{
	...
	return transform_binop(node, new_bd_loongarch64_Shl);
}
...
static ir_node *gen_Shr(ir_node *node)
{
	...
	if (get_mode_modulo_shift(mode) != 32)
		panic("modulo shift!=32 not supported");
	return transform_binop(node, new_bd_loongarch64_Shr);
}
...
static ir_node *gen_Add(ir_node *node)
{
	...
	if (mode_is_float(mode)) {
		return transform_binop(node, new_bd_loongarch64_fAdd);
	}
	...
	return transform_binop(node, new_bd_loongarch64_Add);
}
...
static ir_node *gen_Sub(ir_node *node)
{
	...
	if (mode_is_float(mode)) {
		return transform_binop(node, new_bd_loongarch64_fSub);
	}
	return transform_binop(node, new_bd_loongarch64_Sub);
}
...
static ir_node *gen_Mul(ir_node *node)
{
	...
	if (mode_is_float(mode)) {
		return transform_binop(node, new_bd_loongarch64_fMul);
	}
	return transform_binop(node, new_bd_loongarch64_Mul);
}
...
static ir_node *gen_Minus(ir_node *node)
{
	...
	if (mode_is_float(mode)) {
		return transform_unop(node, n_Minus_op, new_bd_loongarch64_fMinus);
	}
	return transform_unop(node, n_Minus_op, new_bd_loongarch64_Minus);
}

static ir_node *gen_Not(ir_node *node)
{
	return transform_unop(node, n_Not_op, new_bd_loongarch64_Not);
}
...
static ir_node *gen_Load(ir_node *node)
{
	...
	if (mode_is_float(mode)) {
		return new_bd_loongarch64_fLoad(dbgi, new_block, new_mem, new_ptr);
	}
	return new_bd_loongarch64_Load(dbgi, new_block, new_mem, new_ptr);
}
...
static ir_node *gen_Store(ir_node *node)
{
	...
	if (mode_is_float(mode)) {
		return new_bd_loongarch64_fStore(dbgi, new_block, new_mem, new_ptr, new_val);
	}
	return new_bd_loongarch64_Store(dbgi, new_block, new_mem, new_ptr, new_val);
}
...
static ir_node *gen_Jmp(ir_node *node)
{
	ir_node  *new_block = be_transform_nodes_block(node);
	dbg_info *dbgi      = get_irn_dbg_info(node);
	return new_bd_loongarch64_Jmp(dbgi, new_block);
}
...
{
	be_start_out outs[N_LOONGARCH64_REGISTERS] = { [REG_SP] = BE_START_IGNORE };
...
{
	int                               p     = n_loongarch64_Return_first_result;
	...
	in[n_loongarch64_Return_mem]   = be_transform_node(get_Return_mem(node));
	reqs[n_loongarch64_Return_mem] = arch_memory_req;

	in[n_loongarch64_Return_stack]   = get_irg_frame(irg);
...
static ir_node *gen_Proj_Load(ir_node *node)
{
	...
	case pn_Load_M:
		return be_new_Proj(new_load, pn_loongarch64_Load_M);
	case pn_Load_res:
		return be_new_Proj(new_load, pn_loongarch64_Load_res);
...
static ir_node *gen_Proj_Start(ir_node *node)
{
	...
	case pn_Start_P_frame_base:
		return be_get_Start_proj(irg, &loongarch64_registers[REG_SP]);
	}
...
static void setup_calling_convention(ir_graph *irg)
{
	...
	unsigned *allocatable_regs = rbitset_obstack_alloc(obst, N_LOONGARCH64_REGISTERS);
	rbitset_set_all(allocatable_regs, N_LOONGARCH64_REGISTERS);
...
{
	FIRM_DBG_REGISTER(dbg, "firm.be.loongarch64.transform");
}
